...
class PointmassEnv(Env):
    ...
    def __init__(self,
        ...
        self.observation_space = Box(-1.0 * np.ones([size * 2]),
                                     np.ones([size * 2]))
        self.action_space = Box(-1.0 * np.ones([size]),
                                np.ones([size]))
        ...
        self.goal = np.random.uniform(low=-1.0,
                                      high=1.0,
                                      size=[self.size])
    ...
    def step(self,
        ...
        reward = -1.0 * np.linalg.norm(self.position - self.goal,
                                       ord=self.order)

        return np.concatenate([
            self.position, self.goal], 0), reward, False, {}
